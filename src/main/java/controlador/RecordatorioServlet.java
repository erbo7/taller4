        ...
        try {
            switch (accion) {
                ...
                case "registrarRespuesta":
                    registrarRespuestaCliente(request, response);
                    break;
...
// ==============================================
    private void listarRecordatorios(HttpServletRequest request, HttpServletResponse response)
        ...
        if (recordatorios.isEmpty()) {
            ...
            for (Recordatorio rec : recordatorios) {
                ...
                // ========================================
                // COLUMNA DE RESPUESTA (NUEVA)
                // ========================================
                html.append("<td>");
                if (rec.getRespuestaCliente() != null && !rec.getRespuestaCliente().isEmpty()) {
                    String respuestaClass = "";
                    if ("CONFIRMADA".equals(rec.getRespuestaCliente())) {
                        respuestaClass = "badge-enviado";
                    } else if ("CANCELADA".equals(rec.getRespuestaCliente())) {
                        respuestaClass = "badge-fallado";
                    }
                    html.append("<span class='badge-estado ").append(respuestaClass).append("'>")
                            .append(rec.getRespuestaCliente())
                            .append("</span>");
                } else {
                    html.append("<span class='text-muted'>Sin respuesta</span>");
                }
                html.append("</td>");
                ...
                // ========================================
                html.append("<td>");
                if (rec.getEstadoEnvio() != null && "ENVIADO".equals(rec.getEstadoEnvio())
                        && (rec.getRespuestaCliente() == null || rec.getRespuestaCliente().isEmpty())) {
                    html.append("<button class='btn btn-sm btn-success me-1' ")
                            .append("onclick='registrarRespuesta(").append(rec.getIdRecordatorio()).append(", \"1\")' ")
                            .append("title='Confirmar'>")
                            .append("<i class='fas fa-check'></i>")
                            .append("</button>")
                            .append("<button class='btn btn-sm btn-danger' ")
                            .append("onclick='registrarRespuesta(").append(rec.getIdRecordatorio()).append(", \"2\")' ")
                            .append("title='Cancelar'>")
    ...
    private void actualizarConfiguracion(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Obtener parámetros
            String idConfigStr = request.getParameter("idConfig"); // ¡IMPORTANTE!
            ...
            System.out.println("  idConfig: " + idConfigStr); // Debug
            ...
            if (idConfigStr == null || idConfigStr.trim().isEmpty()) {
                response.getWriter().write("ERROR|No se proporcionó el ID de configuración");
                return;
            }
            ...
            // Crear y actualizar configuración CON EL ID
            RecordatorioConfig config = new RecordatorioConfig();
            config.setIdConfig(idConfig); // ¡AGREGAR ESTA LÍNEA!
            ...
            System.out.println("Config a actualizar - ID: " + config.getIdConfig()
                    + ", Canal: " + config.getCanalPrincipal());
    ...
    // ==============================================
// 11. REGISTRAR RESPUESTA DEL CLIENTE
// ==============================================
    private void registrarRespuestaCliente(HttpServletRequest request, HttpServletResponse response)
            throws Exception {

        try {
            String idRecordatorioStr = request.getParameter("idRecordatorio");
            String respuesta = request.getParameter("respuesta");

            if (idRecordatorioStr == null || respuesta == null) {
            ...
            boolean exito = dao.registrarRespuestaCliente(idRecordatorio, respuestaTexto);

            if (exito) {
                response.getWriter().write("OK|Respuesta registrada correctamente");
            } else {
                response.getWriter().write("ERROR|No se pudo registrar la respuesta");
            }
