...
public class DashboardVeteDAO {
    ...
    public int obtenerCitasHoy(int idVeterinario) {
        int cantidad = 0;
        String sql = "SELECT COUNT(*) AS total FROM citas ci JOIN agenda_slots s ON ci.ID_SLOT = s.ID_SLOT WHERE ci.ID_VETERINARIO = ? AND DATE(s.FECHA_HORA_INICIO) = CURDATE() AND ci.ESTADO != 'CANCELADA'";
    ...
    public int obtenerConsultasEnCurso(int idVeterinario) {
        int cantidad = 0;
        String sql = "SELECT COUNT(*) AS total FROM consultas WHERE ID_VETERINARIO = ? AND ESTADO = 'EN_CURSO'";
    ...
    // === OBTENER CANTIDAD DE ESTUDIOS Y ANÁLISIS PENDIENTES ===
    public int obtenerEstudiosPendientes(int idVeterinario) {
        int cantidad = 0;
        String sql = "SELECT (SELECT COUNT(*) FROM ordenes_estudios oe JOIN consultas c ON oe.ID_CONSULTA = c.ID_CONSULTA WHERE c.ID_VETERINARIO = ? AND oe.ESTADO = 'SOLICITADA') + (SELECT COUNT(*) FROM ordenes_analisis oa JOIN consultas c ON oa.ID_CONSULTA = c.ID_CONSULTA WHERE c.ID_VETERINARIO = ? AND oa.ESTADO = 'SOLICITADA') AS total";
    ...
    public int obtenerPacientesAtendidosMes(int idVeterinario) {
        int cantidad = 0;
        String sql = "SELECT COUNT(DISTINCT ID_MASCOTA) AS total FROM consultas WHERE ID_VETERINARIO = ? AND MONTH(FECHA_HORA_INICIO) = MONTH(CURDATE()) AND YEAR(FECHA_HORA_INICIO) = YEAR(CURDATE()) AND ESTADO = 'FINALIZADA'";
    ...
    public List<Map<String, Object>> obtenerAgendaHoy(int idVeterinario) {
        ...
        String sql = "SELECT ci.ID_CITA, s.FECHA_HORA_INICIO, ci.MOTIVO, ci.ESTADO, m.NOMBRE AS mascota, r.NOMBRE AS raza, cl.NOMBRE AS cliente FROM citas ci JOIN agenda_slots s ON ci.ID_SLOT = s.ID_SLOT JOIN mascotas m ON ci.ID_MASCOTA = m.ID_MASCOTA LEFT JOIN razas r ON m.ID_RAZA = r.ID_RAZA JOIN clientes cl ON m.ID_CLIENTE = cl.ID_CLIENTE WHERE ci.ID_VETERINARIO = ? AND DATE(s.FECHA_HORA_INICIO) = CURDATE() AND ci.ESTADO != 'CANCELADA' ORDER BY s.FECHA_HORA_INICIO";
