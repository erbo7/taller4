...
public class ConsultaDAO {
    ...
    public String finalizarConsulta(int idConsulta) {
        ...
        try {
            ...
            // 1️⃣ Actualizar estado de la consulta a FINALIZADA
            String sqlConsulta = """
            UPDATE consultas 
            ...
            // 2️⃣ ⭐ NUEVO: Actualizar también el estado de la CITA a FINALIZADA
            String sqlCita = """
            UPDATE citas 
            SET ESTADO = 'FINALIZADA'
            WHERE ID_CITA = (SELECT ID_CITA FROM consultas WHERE ID_CONSULTA = ?)
            """;
            PreparedStatement psCita = conn.prepareStatement(sqlCita);
            psCita.setInt(1, idConsulta);
            psCita.executeUpdate();
            System.out.println("✅ Estado de CITA cambiado a FINALIZADA");
