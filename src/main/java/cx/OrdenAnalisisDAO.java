...
public class OrdenAnalisisDAO {
    ...
    public List<Map<String, Object>> obtenerConsultasFinalizadas(int idMascota) {
        ...
        String sql = "SELECT "
                ...
                + "    COUNT(oa.ID_ANALISIS) AS total_ordenes "
                + "FROM consultas c "
                + "JOIN veterinarios v ON c.ID_VETERINARIO = v.ID_VETERINARIO "
                + "LEFT JOIN ordenes_analisis oa ON c.ID_CONSULTA = oa.ID_CONSULTA AND oa.ESTADO = 'EMITIDA' "
    ...
    // === LISTAR ORDENES DE ANÁLISIS DE UNA MASCOTA ===
    public List<Map<String, Object>> listarOrdenes(int idMascota) {
        ...
        String sql = "SELECT "
                ...
                + "    v.NOMBRE AS veterinario "
                + "FROM ordenes_analisis oa "
                + "JOIN tipo_analisis ta ON oa.ID_TIPO_ANALISIS = ta.ID_TIPO_ANALISIS "
                + "JOIN consultas c ON oa.ID_CONSULTA = c.ID_CONSULTA "
                + "JOIN veterinarios v ON oa.ID_VETERINARIO = v.ID_VETERINARIO "
                + "WHERE oa.ID_MASCOTA = ? "
                + "ORDER BY oa.FECHA_ORDEN DESC";
        ...
        try (Connection conn = conexionDB.conectar(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                orden.put("idOrden", rs.getInt("ID_ANALISIS"));
                orden.put("fechaOrden", rs.getTimestamp("FECHA_ORDEN"));
                orden.put("tipoAnalisis", rs.getString("tipo_analisis"));
    ...
    public List<Map<String, Object>> obtenerTiposAnalisis() {
        List<Map<String, Object>> tipos = new ArrayList<>();
        String sql = "SELECT ID_TIPO_ANALISIS, NOMBRE FROM tipo_analisis ORDER BY NOMBRE";
        ...
        try (Connection conn = conexionDB.conectar(); PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                tipo.put("idTipo", rs.getInt("ID_TIPO_ANALISIS"));
    ...
    // === REGISTRAR ORDEN DE ANÁLISIS ===
    public boolean registrarOrden(int idConsulta, int idMascota, int idVeterinario,
            int idTipoAnalisis, String motivo, String observaciones) {
        String sql = "INSERT INTO ordenes_analisis "
                + "(ID_CONSULTA, ID_MASCOTA, ID_VETERINARIO, ID_TIPO_ANALISIS, MOTIVO, OBSERVACIONES) "
                + "VALUES (?, ?, ?, ?, ?, ?)";
    ...
    // === EDITAR ORDEN DE ANÁLISIS ===
    public boolean editarOrden(int idOrden, int idTipoAnalisis, String motivo, String observaciones) {
        String sql = "UPDATE ordenes_analisis SET "
                + "ID_TIPO_ANALISIS = ?, "
                + "MOTIVO = ?, "
                + "OBSERVACIONES = ?, "
                + "VERSION = VERSION + 1 "
                + "WHERE ID_ANALISIS = ? AND ESTADO = 'EMITIDA'";

        try (Connection conn = conexionDB.conectar(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, idTipoAnalisis);
