...
public class CitaDAO {
    ...
    public String confirmar(int idCita, int modificadoPor) {
        String sql = "UPDATE citas SET estado = 'CONFIRMADA', "
                + "modificado_por = ?, modificado_en = NOW() "
                + "WHERE id_cita = ? AND estado = 'RESERVADA'";
        ...
        try (Connection conn = conexionDB.conectar(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            if (affectedRows > 0) {
                ...
                return "ERROR|No se pudo confirmar la cita (puede que ya esté confirmada o no exista)";
            }
