    ...
    <body>
        ...
        <script>
                            ...
                            function guardarConfiguracion() {
                                ...
                                var idConfig = form.idConfig.value; // ¡IMPORTANTE! Faltaba esto
                                ...
                                var habilitado = form.habilitado ? (form.habilitado.checked ? 'on' : 'off') : 'off';

                                console.log('DEBUG - Valores capturados:');
                                console.log('  idConfig:', idConfig);
                                console.log('  horasAntes:', horasAntes);
                                console.log('  horaEnvio:', horaEnvio);
                                console.log('  canalPrincipal:', canalPrincipal);
                                console.log('  maxReintentos:', maxReintentos);
                                console.log('  habilitado:', habilitado);

                                if (!idConfig || !horasAntes || !horaEnvio || !plantilla || !canalPrincipal) {
                                    mostrarMensaje('warning', 'Complete todos los campos obligatorios');
                                    return;
                                }

                                // Construir URL con parámetros - INCLUIR idConfig
                                var url = 'RecordatoriosServlet?accion=actualizarConfig' +
                                        '&idConfig=' + encodeURIComponent(idConfig) +
                            ...
                            // ===================== REGISTRAR RESPUESTA =====================
                            function registrarRespuesta(idRecordatorio, respuesta) {
                                var textoRespuesta = respuesta === "1" ? "confirmar la asistencia" : "cancelar la cita";

                                if (!confirm('¿Desea ' + textoRespuesta + ' para este recordatorio?'))
                                    return;

                                fetch('RecordatoriosServlet?accion=registrarRespuesta&idRecordatorio=' + idRecordatorio + '&respuesta=' + respuesta)
                                        .then(function (r) {
                                            return r.text();
                                        })
                                        .then(function (respuesta) {
                                        ...
                                        .catch(function (err) {
                                            console.error('Error:', err);
                                            mostrarMensaje('danger', 'Error al registrar respuesta');
                                        });
                            }
        </script>
    </body>
